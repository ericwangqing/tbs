...
.config-form
  AForm(layout="vertical")
    AFormItem(label="Name")
      AInput.w-760px(size="large" v-model:value="data.name")
  AForm(layout="vertical")
  ...
  .tx-count(v-if="txnLoading || data.dataset?.txCount")
    ...
    span(v-if="data.dataset.txCount || data.dataset.txCount === 0") {{ thousands(data.dataset.txCount) }}
    ASpin(v-else)
  .divide-line
  AForm(layout="vertical")
    AFormItem(label="Shards")
      AInputNumber.w-260px(size="large" v-model:value="data.shards")
    AFormItem(label="Node Scale")
      AInputNumber.w-260px(size="large" :formatter="thousands" v-model:value="data.nodes")
    AFormItem(label="Latency In Shard(ms)")
      AInputNumber.w-260px(size="large" addon-after="ms" :formatter="thousands")
    AFormItem(label="Latency Between Shard")
      AInputNumber.w-260px(size="large" addon-after="ms" :formatter="thousands")
    AFormItem(label="Throughput")
      AInputNumber.w-260px(size="large" addon-after="TPS" :formatter="thousands")
...
import { thousands, formatTime } from '../composition/util.js'
