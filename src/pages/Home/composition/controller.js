...
import addressTool from './addr-generator'
...
class Controller extends EventEmitter {
  ...
  initChart({ worldMapChart, performanceChart, resourceChart, withAttacker, bandwidthGauge, memoryGauge, cpuGauge }) {
    if (worldMapChart) this.worldMapChart = worldMapChart
  ...
  _redrawNetwork() {
   ...
   // const swarms = _.sampleSize(this.swarms, Math.ceil(Math.sqrt(this.swarms.length)));
    ...
    const generatedData = this.worldMapChartData.map((d) => {
      return {
        coords: [d.from, d.to],
        lineStyle: { color: d.isAttackMsg ? '#FF4343' : '#FFFFFF' },
        opacity: 1
      }
    })
    redrawNetworkDebounce(this.worldMapChart, generatedData)
  }

  _drawTx({ arrivalTime, from, to, isAttackMsg }) {
    const isTxLineExist = this.worldMapChartData.find((data) => (data.from === from && data.to === to))
    if (isTxLineExist) {
      isTxLineExist.arrivalTime = arrivalTime
    } else {
      const chartDataLength = this.worldMapChartData.length
      this.worldMapChartData.unshift({ arrivalTime, from, to, isAttackMsg })
      this.worldMapChartData = _.uniqBy(this.worldMapChartData, (data) => (data.from + data.to))
      if (chartDataLength < this.worldMapChartData.length) this._redrawNetwork()
    }
  }
